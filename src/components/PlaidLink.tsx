...
export default function PlaidLinkSection() {
  ...
  const [view, setView] = useState<"summary" | "review" | "history" | "transfers">(
    "summary"
  );
  const [statementFilter, setStatementFilter] = useState<string>("all");
  const [statements, setStatements] = useState<{id: string; file_name: string; bank_name: string}[]>([]);

  const fetchPlaidData = useCallback(async () => {
    try {
      const [plaidRes, expRes, stmtRes] = await Promise.all([
  ...
  const allNeedsReview = transactions.filter(
    (t) => t.review_status === "needs_review"
  );
  // Apply statement filter
  const needsReview = statementFilter === "all"
    ? allNeedsReview
    : statementFilter === "plaid"
    ? allNeedsReview.filter((t) => t.source === "plaid")
    : allNeedsReview.filter((t) => t.statement_id === statementFilter);

  // Get unique statements that have transactions needing review
  const reviewStatementIds = [...new Set(allNeedsReview.filter((t) => t.statement_id).map((t) => t.statement_id))];
  const reviewStatements = statements.filter((s) => reviewStatementIds.includes(s.id));
  const hasPlaidTxns = allNeedsReview.some((t) => t.source === "plaid");
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-white rounded-2xl border border-porch-cream-dark/50 p-4">
        ...
        {isConnected ? (
          <>
            ...
            {/* Review badge */}
            {allNeedsReview.length > 0 && view !== "review" && (
              <button
              ...
              >
                {allNeedsReview.length} transaction
                {allNeedsReview.length !== 1 ? "s" : ""} need your approval
              </button>
            )}
          ...
          <>
            <p className="text-xs text-porch-brown-light/60 mb-3">
              Connect your bank account to automatically import and categorize
              transactions. The app learns from your approvals and gets smarter
              over time!
            </p>
            <button
      ...
      {/* REVIEW PANEL — Fast one-tap approve with changeable category */}
      {view === "review" && allNeedsReview.length > 0 && (
        <div className="bg-white rounded-2xl border border-porch-cream-dark/50 overflow-hidden">
          <div className="px-4 py-2.5 border-b border-porch-cream-dark/30">
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <p className="text-[10px] text-porch-brown-light/40 mt-0.5">
                  {allNeedsReview.length} total — showing {needsReview.length}
                </p>
              </div>
              <button
                onClick={approveAll}
            ...
            {/* Statement filter — lets you work through one PDF at a time */}
            {(reviewStatements.length > 1 || hasPlaidTxns) && (
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-colors ${
                    statementFilter === "all"
                      ...
                      : "bg-porch-cream/50 text-porch-brown-light/60"
                  }`}
                >
                  All ({allNeedsReview.length})
                </button>
                {hasPlaidTxns && (
                  <button
                    onClick={() => setStatementFilter("plaid")}
                    className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-colors ${
                      statementFilter === "plaid"
                        ? "bg-porch-teal text-white"
                        : "bg-porch-cream/50 text-porch-brown-light/60"
                  ...
                  >
                    Bank ({allNeedsReview.filter((t) => t.source === "plaid").length})
                  </button>
                )}
                {reviewStatements.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => setStatementFilter(s.id)}
                    className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-colors ${
                      statementFilter === s.id
                        ? "bg-porch-teal text-white"
                        : "bg-porch-cream/50 text-porch-brown-light/60"
                    }`}
                  >
                    {s.bank_name || s.file_name} ({allNeedsReview.filter((t) => t.statement_id === s.id).length})
                  </button>
                ))}
              </div>
            )}
      ...
      {/* TRANSFERS PANEL — Auto-detected transfers (CC payments, bank transfers, etc.) */}
      {view === "transfers" && (
        <div className="bg-white rounded-2xl border border-porch-cream-dark/50 overflow-hidden">
          <div className="px-4 py-2.5 border-b border-porch-cream-dark/30">
            <h3 className="text-xs font-bold text-porch-brown uppercase tracking-wider">
 ...
 * Shows merchant, amount, recommended category (tappable to change), and approve button
 */
function ReviewCard({
  txn,
  categories,
  ...
  categories: Category[];
  onApprove: (txnId: string, catId: string, catName: string, isSoftExpense: boolean) => void;
}) {
  ...
  const [softExpense, setSoftExpense] = useState(false);

  // Auto-approve when category changes (from dropdown selection)
  ...
  useEffect(() => {
    ...
    setApproving(true);
    onApprove(txn.transaction_id, pendingApprove, catName, softExpense);
  ...
  // Shorten category names for the button display (full name in dropdown)
  ...
  return (
    <div className="px-3 py-2.5">
      {/* Single row: Merchant | Amount | Category | Soft | Approve */}
      <div className="flex items-center gap-1.5">
        {/* Merchant + date — takes more space now that category is shorter */}
        <div className="min-w-0 flex-1">
          <p className="text-xs font-medium text-porch-brown truncate">
            {txn.merchant_name || txn.name}
            {txn.source === "statement" && (
              <span className="ml-1 inline-block px-1 py-0.5 bg-purple-100 text-purple-600 text-[8px] font-semibold rounded align-middle">
                PDF
              </span>
            )}
          </p>
        ...
        {/* Category — tappable, with shortened names */}
        {!showPicker ? (
          <button
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setShowPicker(false);
              // Trigger auto-approve via useEffect (async onChange gets killed by re-render)
              if (newCatId) {
        ...
        {/* Soft expense toggle */}
        <button
          onClick={() => setSoftExpense(!softExpense)}
          title="Soft expense"
          className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-colors ${
            softExpense
              ? "bg-amber-100 border border-amber-300"
              : "border border-porch-cream-dark/30 hover:border-porch-cream-dark/50"
          }`}
        >
          <span className="text-[10px]">{softExpense ? "S" : "H"}</span>
        </button>
        ...
        <button
          onClick={async () => {
            ...
            setApproving(true);
            await onApprove(txn.transaction_id, selectedCat, selectedName, softExpense);
            setApproving(false);
          }}
          disabled={approving}
